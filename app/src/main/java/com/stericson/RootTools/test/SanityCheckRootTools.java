...
public class SanityCheckRootTools extends Activity {
	...
	private class SanityCheckThread extends Thread {
		...
		public void run() {
			 ...
			 * RootTools.installBinary(mContext, R.raw.nes, "nes_binary")) {
			 * visualUpdate(TestHandler.ACTION_HIDE,
			 * "ERROR: Failed to install binary. Please see log file."); return;
			 * }
			 */
			...
			visualUpdate(TestHandler.ACTION_DISPLAY,
					"[ Checking busybox is setup ]\n");
			visualUpdate(TestHandler.ACTION_DISPLAY,
					RootTools.checkUtil("busybox") + " k\n\n");

			visualUpdate(TestHandler.ACTION_PDISPLAY,
					"Testing getBusyBoxVersion");
			visualUpdate(TestHandler.ACTION_DISPLAY,
					"[ Checking busybox version ]\n");
			visualUpdate(TestHandler.ACTION_DISPLAY,
					RootTools.getBusyBoxVersion("/system/bin/") + " k\n\n");
			...
			try {
				...
				visualUpdate(
						...
						RootTools.fixUtils(new String[] { "ls", "rm", "ln",
								"dd", "chmod", "mount" })
								+ " k\n\n");
			} catch (Exception e2) {
				// TODO Auto-generated catch block
			...
			Permissions permissions = RootTools
					.getFilePermissionsSymlinks("/system/bin/busybox");
			visualUpdate(TestHandler.ACTION_DISPLAY,
			...
			visualUpdate(TestHandler.ACTION_DISPLAY,
					"[ Running some Java code ]\n");

			Shell shell;
			try {
				shell = RootTools.getShell(true);
				JavaCommandCapture cmd = new JavaCommandCapture(43, false,
				...
				// Oops. Say, did you run RootClass and move the resulting
				// anbuild.dex " file to res/raw?
