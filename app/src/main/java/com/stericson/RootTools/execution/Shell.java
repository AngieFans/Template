...
public class Shell {
	...
	public static enum ShellContext {
		...
		SYSTEM_SERVER("u:r:system_server:s0"), // system_server, u:r:system:s0
												// on some firmwares
	...
	private Shell(String cmd, ShellType shellType, ShellContext shellContext,
			int shellTimeout) throws IOException, TimeoutException,
            RootDeniedException {
		...
		if (this.shellContext == ShellContext.NORMAL) {
			this.proc = new ProcessBuilder(cmd).redirectErrorStream(true)
					.start();
		} else {
			// only done for root shell...
			this.proc = new ProcessBuilder(cmd, "--context "
					+ this.shellContext.getValue()).redirectErrorStream(true)
					.start();
		}
	...
	public static Shell startRootShell(int timeout, ShellContext shellContext,
                                       int retry) throws IOException, TimeoutException,
            RootDeniedException {
		...
		if (Shell.rootShell == null) {
			...
			while (Shell.rootShell == null) {
				try {
					Shell.rootShell = new Shell(cmd, ShellType.ROOT,
							shellContext, timeout);
				} catch (IOException e) {
					if (retries++ >= retry) {
			...
			try {
				RootTools
						.log("Context is different than open shell, switching context...");
				Shell.rootShell.switchRootShellContext(shellContext);
			} catch (IOException e) {
				RootTools
						.log("Context could not be switched for existing root shell...");
				throw e;
			}
		} else {
	...
	public static Shell startCustomShell(String shellPath, int timeout)
		...
		if (Shell.customShell == null) {
			...
			Shell.customShell = new Shell(shellPath, ShellType.CUSTOM,
					ShellContext.NORMAL, timeout);
		} else
	...
	public static Shell startShell(int timeout) throws IOException,
		...
		try {
			if (Shell.shell == null) {
				...
				Shell.shell = new Shell("/system/bin/sh", ShellType.NORMAL,
						ShellContext.NORMAL, timeout);
			} else
	...
	public Shell switchRootShellContext(ShellContext shellContext)
		...
		if (this.shellType == ShellType.ROOT) {
			try {
				...
				RootTools
						.log("Problem closing shell while trying to switch context...");
			}
