...
import com.ccmt.template.accessibility.domain.NodeInfo;
...
abstract class AbstractAccessibiltyJob implements IAccessibilityJob {
    ...
    @SuppressWarnings("unused")
    private static final String TIP = CcmtApplication.application.getResources().getString(R.string.app_name);
...
//        mKeyguardManager = (KeyguardManager) mService.getSystemService(Context.KEYGUARD_SERVICE);
    ...
    public void onReceiveJob(AbstractAccessibilityService accessibilityService, AccessibilityEvent event) {
        ...
        CharSequence classNameCharSequence = event.getClassName();
        LogUtil.i("classNameCharSequence -> " + classNameCharSequence);
        if (classNameCharSequence == null) {
            // 在捕捉的界面中如果有其他非捕捉的界面弹出来,classNameCharSequence就为null.
            return;
        }
        String className = classNameCharSequence.toString();
