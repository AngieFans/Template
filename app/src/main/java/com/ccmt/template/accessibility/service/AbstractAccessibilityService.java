package com.ccmt.template.accessibility.service;
...
//        lruMap.put(getClass().getName(), this);
        String name = AbstractAccessibilityService.class.getName();
        List<AbstractAccessibilityService> accessibilityServices = (List<AbstractAccessibilityService>) lruMap.get(name);
        if (accessibilityServices == null) {
            accessibilityServices = new ArrayList<>();
            lruMap.put(name, accessibilityServices);
        }
        if (!accessibilityServices.contains(this)) {
            accessibilityServices.add(this);
        }
        lruMap.put(name, this);
...
//        lruMap.remove(getClass().getName(), false);
        String name = AbstractAccessibilityService.class.getName();
        List<AbstractAccessibilityService> accessibilityServices = (List<AbstractAccessibilityService>) lruMap.get(name);
        if (accessibilityServices != null) {
            int size = accessibilityServices.size();
            if (size > 0) {
                if (accessibilityServices.remove(this)) {
                    if (size == 1) {
                        lruMap.remove(name, false);
                    }
                }
            }
        }
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        String pkn = event.getPackageName().toString();
        Log.i("MyLog", "pkn -> " + pkn);
        if (mAccessbilityJobs != null && !mAccessbilityJobs.isEmpty()) {
            for (IAccessibilityJob job : mAccessbilityJobs) {
    ...
    public boolean isNotificationServiceRunning() {
        return AbstractNotificationService.isRunning(this);
    }
