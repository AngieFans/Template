...
public class Shell {
     ...
     * @return Output of the commands, or null in case of an error
     */
    @SuppressWarnings({"TryWithIdenticalCatches", "StatementWithEmptyBody"})
...
//        Debug.logCommand(String.format("[%s%%] END", shell.toUpperCase(Locale.ENGLISH)));
    ...
    public static class Interactive {
        ...
        private volatile boolean mIsStartRootDone;
...
//                        watchdogTimeout = builder.watchdogTimeout;
        ...
        private synchronized void handleWatchdog() {
            LogUtil.i("RootUtil.isStartRootDone -> " + RootUtil.isStartRootDone);
            ...
            if (RootUtil.isStartRootDone) {
                synchronized (RootUtil.obj7) {
                    if (RootUtil.isStartRootDone) {
                        if (!mIsStartRootDone) {
                            mIsStartRootDone = true;
                        }

                        return;
                    }
                    postCallback(command, OnCommandResultListener.SHELL_DIED, buffer);
            ...
            if (running && idle && (commands.size() > 0)) {
                ...
                if (command.commands.length > 0) {
                    try {
                        ...
                        if (Arrays.binarySearch(command.commands, "echo -BOC-") >= 0) {
