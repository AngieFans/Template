...
public abstract class AbstractUserPermissionsCheckFragment extends AbstractFragment {
    ...
    protected void restoreState(View view, @Nullable Bundle savedInstanceState) {
        super.restoreState(view, savedInstanceState);
        AbstractUserPermissionsCheckFragment fragment = (AbstractUserPermissionsCheckFragment) sFragment;
        mShowView = fragment.mShowView;
        mHideView = fragment.mHideView;
        mIsClickButton = fragment.mIsClickButton;
        mIsShowHasNoNonDynamicPermissionsContented = fragment.mIsShowHasNoNonDynamicPermissionsContented;
        mIsShowHasNoDynamicPermissionsContented = fragment.mIsShowHasNoDynamicPermissionsContented;
        mIsShowHasDynamicPermissionsContented = fragment.mIsShowHasDynamicPermissionsContented;
        mIsOnPermissionDeniedInvoke = fragment.mIsOnPermissionDeniedInvoke;
        mGrantedPermissionsees = fragment.mGrantedPermissionsees;
        mIsHasNonDynamicPermissions = fragment.mIsHasNonDynamicPermissions;
        mIsAllGranted = fragment.mIsAllGranted;
    }
...
//        LogUtil.i("sIsShowDynamicPermissionDialoged -> " + sIsShowDynamicPermissionDialoged);
        ...
        boolean hasNonDynamicPermissions = isHasNonDynamicPermissions(activity);
...
//                DialogFractory.showProgressDialog(mContext, true);
                DialogFractory.showFullScreenProgressDialog(activity);
            }
...
//        requestDynamicPermissions(false);
    ...
    public static void showPackageUsageStatsDialog(AbstractUserPermissionsCheckFragment userPermissionsCheckFragment) {
        if (sIsShowPackageUsageStatsDialoged != null) {
            return;
        }
        FragmentActivity activity = userPermissionsCheckFragment.getActivity();
        DialogInterface.OnClickListener onClickListener = (dialog, which) -> {
            if (which == DialogInterface.BUTTON_POSITIVE) {
                LogUtil.i("让用户去授权");
                sIsAuthorization = true;
                PermissionsUtil.requestReadNetworkStats(activity);
            } else {
                userPermissionsCheckFragment.showHasNoNonDynamicPermissionsContent();
                if (userPermissionsCheckFragment.isDoOnResumeAfterRequestNonDynamicPermissionsError()) {
                    userPermissionsCheckFragment.doOnResume();
                }
            }
        };
        try {
            PackageManager packageManager = activity.getPackageManager();
            CharSequence label = packageManager.getApplicationInfo(activity.getPackageName(),
                    0).loadLabel(packageManager);
            CustomAlertDialog dialog = new CustomAlertDialog.Builder(activity)
                    .setTitle(activity.getString(R.string.traffic_single_package_usage_stats_title))
                    .setMessage("“" + label.toString() + "”" + activity.getString(R.string
...
//                startService(new Intent(AbstractUserPermissionsCheckActivity.this, TrafficService.class));
     ...
     */
    @SuppressWarnings("JavaDoc")
    protected boolean isDoOnResumeAfterRequestNonDynamicPermissionsError() {
        return true;
    }
...
//     */
//    @SuppressWarnings("JavaDoc")
//    protected boolean isReturnAfterRequestNonDynamicPermissionsError() {
//        return true;
//    }
